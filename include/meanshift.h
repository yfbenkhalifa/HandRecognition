...
class MeanShift {
  private:
    ...
    std::vector<Cluster> RGClustering(const Sample *samples);

    vector<Point> GrowRegion(const Sample *shifted_points, const vector<Point> &points, int *mask, const int &clusterIndex);

  public:
    MeanShift(const Mat &_image, const int &_spatial_bandwidth = 4, const double &_color_bandwidth = 4) {
        image = _image.clone();
