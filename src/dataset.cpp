...
vector<int> Dataset::subVector(vector<int> const &v, int m, int n)
{
...
bool Dataset::computeMask(Mat src, Mat dst, Mat mask)
{
    Vec3b threshold = (200, 200, 200);
    Vec3b nullValue = (0, 0, 0);
...
Mat Dataset::applyMask(Mat src, Mat mask)
{
    Mat dst = src.clone();
    Vec3b threshold = (200, 200, 200);
    Vec3b nullValue = (0, 0, 0);
...
void Dataset::subMatrix(Mat src, Mat dst, int posX, int posY, int sizeX, int sizeY)
{
...
vector<string> Dataset::readPath(char *folderPath)
{
    DIR *dir;
...
vector<Mat> Dataset::readSrcImages(char *folderPath, int maxSize)
{
...
vector<Mat> Dataset::readMaskImages(char *folderPath, int maxSize)
{
    vector<string> files = readPath(folderPath);
    vector<Mat> masks;
    Mat temp;
...
vector<vector<int>> Dataset::readCsvFiles(char *folderPath, int maxSize)
{
    vector<string> files = readPath(folderPath);
...
string Dataset::vectorToString(vector<int> vec)
{
    string s;
    for (auto x : vec)
        s += x;
...
HandMetadata Dataset::loadHandMetadata(vector<int> coords)
{
    HandMetadata temp;
...
Mat Dataset::cutImage(Mat src, HandMetadata boundingBox)
{
...
vector<string> Dataset::splitstr(string str, string deli)
{
    vector<string> vec;
    int start = 0;
...
string Dataset::readFileIntoString(const string &path)
{
...
void Dataset::createDataset(int maxSize)
{
    vector<Mat> images;
